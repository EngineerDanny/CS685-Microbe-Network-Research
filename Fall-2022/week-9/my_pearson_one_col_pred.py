...
out_file = out_dir + "/pearson_model_one_col_results.csv"
...
# should be in the range of 50s
for index in range(1, total_row_nos, 24):
    sub_data = amgut_data.iloc[:-index, :]
    data_dict[f"amgut{index}"] = sub_data
            ...
            if i != j:
                ...
                if abs_calc_cor >= 0:
                    ...
                    cor_df = pd.concat([cor_df,
                                        pd.DataFrame({'taxa1': sub_data.columns[i],
